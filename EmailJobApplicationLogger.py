...
def extract_job_application_emails(service):
    results = service.users().messages().list(userId='me', maxResults=10, labelIds=['INBOX']).execute()  # Adjust maxResults as needed
    ...
    for message in messages:
        ...
        if date_header:
            ...
            date_time_pattern = r"(\w{3}, \d{1,2} \w{3} \d{4} \d{2}:\d{2}:\d{2})(?: \(\w+\))? \+\d{4}"
            match = re.search(date_time_pattern, raw_date_time)
            if match:
                extracted_date_time = match.group(1)
            parsed_date_time = datetime.strptime(extracted_date_time, "%a, %d %b %Y %H:%M:%S") # Parse raw date string
            formatted_date_time = parsed_date_time.strftime("%d-%m-%Y %H:%M:%S")  # Format date and time
        else:
            formatted_date_time = None

        # Extracting sender's name and email
        sender_header = next((header for header in headers if header['name'] == 'From'), None)
        if sender_header:
